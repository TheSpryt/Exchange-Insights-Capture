...
/**
 * A self-styled search field: a magnifying-glass icon on the left, white text on a dark field, and
 * a red clear "x" that appears once you have typed something.
 *
 * <p>The same component as the Bank Templates panel uses, so the two plugins read as a set. Built
 * directly rather than using the client's IconTextField, whose flat text field renders unreadably
 * in a panel styled like this one - here every colour is under our control, and the rounded body
 * matches the clip cards rather than sitting square among them.
 */
final class SearchBar extends JPanel
{
	...
	SearchBar()
	{
		...
		// Non-opaque so paintComponent can draw the rounded body; the border strokes the matching
		// outline, giving the field the same corner radius as the cards below it.
		setOpaque(false);
	...
	/** A magnifying glass drawn at runtime, so there is no bundled asset to ship and hash-verify. */
