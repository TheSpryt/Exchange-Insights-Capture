...
/**
 * A rounded outline for the panel's controls. It only STROKES the outline - a border
 * paints after the component's own content, so filling here would cover the text.
 * Components that want a rounded body stay non-opaque and paint the fill themselves
 * in paintComponent via {@link #fill}.
 */
final class RoundedBorder implements Border
{
	/** Corner radius shared by every control in the panel. */
